...
class DB:
    ...
    def insert_account(self, account: Account) -> None:
        with self.conn:
            self.c.execute("INSERT INTO all_users VALUES "
                      ...
                      {'ID': account.id, 'name': account.name, 'username': account.username, 'password': account.password,
                       'description': account.description, 'email': account.email, 'is_account_premium':
                           self.bool_to_str(account.account_type == AccountType.PREMIUM), 'user_photo': account.photo, 'is_publisher': account.publisher})
    ...
    def update_account(self, account: Account, updated_fields):
        ...
        with self.conn:
            self.c.execute(query, updated_fields)
    ...
    def insert_playlist(self, playlist: PlayList) -> None:
        ...
        for owner in playlist.owners:
            with self.conn:
                self.c.execute("INSERT INTO person_has_playlist VALUES"
                               "(:user_id, :playlist_id)",
                               {'user_id': owner.id, 'playlist_id': playlist.uid} )
        for music in playlist.musics:
            with self.conn:
                self.c.execute("INSERT INTO music_in_playlist VALUES"
                               "(:music_id, :playlist_id)",
                               {'music_id': music.uid, 'playlist_id': playlist.uid} )
        #print(playlist.uid)
        #print(datetime.now())
        with self.conn:
            self.c.execute("INSERT INTO playlist VALUES"
                           "(:playlist_id, :creator_id, :creation_date)",
                           {'playlist_id': playlist.uid, 'creator_id': playlist.owners[0].id, 'creation_date': datetime.now()})
        pass

    def delete_playlist(self, playlist_id: int) -> None:
        with self.conn:
            self.c.execute("DELETE from playlist WHERE playlist_id:=playlist_id", {'playlist_id': playlist_id})
        with self.conn:
            self.c.execute("DELETE from person_has_playlist WHERE playlist_id:=playlist_id", {'playlist_id': playlist_id})
        with self.conn:
            self.c.execute("DELETE from music_in_playlist WHERE playlist_id:=playlist_id", {'playlist_id': playlist_id})
    ...
    def delete_music_from_playlist(self, playlist_id: int, music_to_remove_id: int):
        with self.conn:
            self.c.execute("DELETE from music_in_playlist WHERE "
    ...
    def insert_music(self, music: Music) -> None:
        ...
        with self.conn:
            self.c.execute("INSERT INTO music VALUES"
